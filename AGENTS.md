...
### Infinity Card product decisions

- Maintain two reusable base themes with the same layout: `women` and `men`.
- Match the approved Medina Editorial visual system: asymmetric hero crop, circular logo, editorial typography, flat grouped rows, thin dividers, and rounded actions.
- Do not show business hours.
- Use a reviews action in place of the directions action.
- Every contact, action, and social item is data-driven and optional. Missing or disabled entries must disappear without leaving layout gaps.
- TikTok is supported as an optional channel but is disabled in the default sample data.
- Keep client content in separate data modules so creating a new client does not require editing the UI implementation.
- Client modules under `src/clients/data/` are auto-discovered; adding a client requires only one new data file and its assets.
- Use one GitHub repository (`infinity-card`) for all public client pages and the phone-first Builder. Each client keeps its data module under `src/clients/data/<slug>/client.ts` and its images under `public/assets/clients/<slug>/`.
- The Builder is available at `?builder=1`, stores local drafts on the current device, and publishes through a secure GitHub bridge when configured. Never expose a GitHub token in the browser bundle.
- The Builder uses native web inputs by deliberate product choice so the simulated keyboard never appears over the editor; the real device keyboard remains available when typing on a phone.
- The Builder includes a 200-slot inventory (`IC-001` through `IC-200`) split into 20 printable lots of 10, local slot statuses (available, reserved, published, sold), a global IC/client search and status filter, a neutral placeholder for unused slots, and local PNG/SVG QR generation for stable slot URLs. Use the Inventory mode to open a lot, mark a physical card sold, and Modifier to customize one exact slot.
- Render the NFC card as a normal responsive web page at every viewport. The simulated iPhone/Pixel frame, picker, status chrome, and camera are hidden; the underlying runtime remains mounted for compatibility.
- Use `npm run build:pages` for a relative-path static build that is safe for GitHub Pages project subdirectories.
- Keep `scripts/prepare-pages-build.mjs` in the Pages build so protected runtime asset URLs remain valid inside a repository subdirectory.
- The deliverable stays local until the user explicitly authorizes publishing or GitHub changes.
